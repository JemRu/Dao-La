...
//处理签到详情搜索
	...
	public function handle_register(){
		...
		$activation_time = time()+60*60*24;//过期时间为24小时后
		$res = $this->sql_model->handle_register($company_email,$password,$regtime,$activation,$activation_time);
		$data['reg_res'] = $res;
	//	$content = "亲爱的用户,您好,感谢您使用到啦，请点此链接激活您的账号".site_url()."/ququer/activate/?activation='$activation'
	//						   如果以上链接无法点击，请将它复制到你的浏览器地址栏中进入访问，该链接24小时内有效。";
		$content = "感谢您在我站注册了新帐号。请点击链接激活您的帐号。"
				.site_url()."ququer/activate/?activation=".$activation."
				如果以上链接无法点击，请将它复制到你的浏览器地址栏中进入访问，该链接24小时内有效。";
		$this->send_email($company_email,$password,$activation,$content);
		$this->load->view('login',$data);
	}
		//检验Email是否可用
	...
	function send_email($email,$password,$activation,$content){
		  $config=Array(
				'crlf'          => "\r\n",
				'newline'       => "\r\n",
				'charset'       => 'utf-8',
	...
	function handle_login(){
		...
		$check_sql = "select * from company where `company_email` = '$company_email' and `company_password` = '$password'";
		$mysql = new SaeMysql();
